...
void wifi_uart_parse(uint8 *wifi_uart_get_data, uint16 length)
{
    char *current = (char*)wifi_uart_get_data;
    while (current - (char*)wifi_uart_get_data < length)
    {
        // 指令以#开头
        ...
        if (platitude != NULL)
        {
            ...
            char *pgratitude = strstr(platitude, "~");
            char *pend = strstr(platitude, "^");
            char latitude[13];
            int llength = pgratitude - platitude - 1;

            memcpy(latitude, platitude + 1, llength);
            latitude[llength] = '\0';
            char gratitude[13];
            int glength = pend - pgratitude - 1;

            memcpy(gratitude, pgratitude + 1, glength);
            gratitude[glength] = '\0';

            // 将经纬度加入fifo中
            location a = {func_str_to_float(latitude), func_str_to_float(gratitude)};
