...
#define LIGHT_PIN_ANALOG (ADC1_CH4_A4)  // 传感器AO 接 板子A4
#define LIGHT_PIN_DIGITAL (B1)          // 传感器DO 接 板子B1
...
int main(void) {
  ...
  LIGHT_init_analog(LIGHT_PIN_ANALOG);
  LIGHT_init_digital(LIGHT_PIN_DIGITAL);
  uint8 analog = 200;
  uint8 digital = 200;
  while (1) {
    
    analog = LIGHT_read_analog(LIGHT_PIN_ANALOG);
    digital = LIGHT_read_digital(LIGHT_PIN_DIGITAL);
    printf("analog: %d, digital: %d \r\n", analog, digital);
